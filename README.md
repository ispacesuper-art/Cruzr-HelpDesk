...
**Current version:** 3.2 · **Package:** `com.usc.cruzr.helpdesk`
...
| `VoiceAssistantController` | AssistantManager keep-alive + pause assistant skills |
| `AssistantSkillPauser` | Pause/stop running Cruzr chat/voice skills |
...
```
app/build/outputs/apk/release/USC-Cruzr-HelpDesk-v3.2-release.apk
release/USC-Cruzr-HelpDesk-v3.2-release.apk
```
...
2. Upload `USC-Cruzr-HelpDesk-v3.2-release.apk`
...
adb install -r "release\USC-Cruzr-HelpDesk-v3.2-release.apk"
```
...
| `quickAsk` | Optional `true` — shows as a yellow pill in the Quick Ask bar |
...
| Stuck on "Retrying microphone access" | Deploy **v2.9+** — no longer waits on competition callbacks. |
| Mic busy / voice assistant interference | **v2.9** — assistant skill pause + burst suppress, then starts listening directly. Tap **Listen** if needed. |
